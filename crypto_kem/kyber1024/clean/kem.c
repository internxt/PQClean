...
#include <string.h>
/*************************************************
* Name:        PQCLEAN_KYBER1024_CLEAN_crypto_kem_keypair_seeded
*
...
* Returns 0 (success)
**************************************************/
int PQCLEAN_KYBER1024_CLEAN_crypto_kem_keypair_seeded(uint8_t *pk,
        uint8_t *sk,
        const uint8_t *coins) {
...
/*************************************************
* Name:        PQCLEAN_KYBER1024_CLEAN_crypto_kem_enc_internal
*
* Description: Generates cipher text and shared
*              secret for given public key
*
* Arguments:   - uint8_t *ct: pointer to output cipher text
*                (an already allocated array of KYBER_CIPHERTEXTBYTES bytes)
*              - const uint8_t *ss: pointer to input shared secret
*                (an already allocated array of KYBER_SSBYTES bytes)
*              - const uint8_t *pk: pointer to input public key
*                (an already allocated array of KYBER_PUBLICKEYBYTES bytes)
**
* Returns 0 (success)
**************************************************/
int PQCLEAN_KYBER1024_CLEAN_crypto_kem_enc_internal(uint8_t *ct,
        const uint8_t *ss,
        const uint8_t *pk) {
    ...
    PQCLEAN_KYBER1024_CLEAN_indcpa_enc(ct, ss, pk, coins);
...
/*************************************************
* Name:         PQCLEAN_KYBER1024_CLEAN_crypto_kem_dec_internal
*
* Description: Generates shared secret for given
*              cipher text and private key without verification
*
* Arguments:   - uint8_t *ss: pointer to output shared secret
*                (an already allocated array of KYBER_SSBYTES bytes)
*              - const uint8_t *ct: pointer to input cipher text
...
*
**************************************************/
int PQCLEAN_KYBER1024_CLEAN_crypto_kem_dec_internal(uint8_t *ss,
        const uint8_t *ct,
        const uint8_t *sk) {

    PQCLEAN_KYBER1024_CLEAN_indcpa_dec(ss, ct, sk);
